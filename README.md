...
-   **Mention the bot**: You can have a conversation with the bot by mentioning it directly (e.g., `@YourBotName What is the capital of France?`) or by pinging an assigned role with less than 5 members (e.g., `@MyAI What is Python?`). Mass mentions like `@everyone` and `@here` are automatically ignored. For large servers (100+ members), set `"enable_members_intent": true` in `settings.json` (and enable Server Members Intent in the Discord Developer Portal) for exact role member counts.
